...
//SPI Flash Erase Sector
    unsigned int
spi_sector_erase(unsigned int uiChip, unsigned int uiAddr) 
...
// SPI Flash Erase Block
    unsigned int
spi_block_erase(unsigned int uiChip, unsigned int uiAddr) 
...
// Erase whole chip
    unsigned int
spi_erase_chip(unsigned int uiChip)  { unsigned int uiRet;
...
/************************************ for old interface ************************************/ 
    unsigned int
spi_read(unsigned int uiChip, unsigned int uiAddr, unsigned int *puiDataOut) 
...
flashread(unsigned long dst, unsigned int src, unsigned long length) 
    {
...
flashwrite(unsigned long dst, unsigned long src, unsigned long length) 
    {  KDEBUG("flashwrite: dst(uiAddr)=%x; src(pucBuffer)=%x; length=%x; \n",
		dst, src, length);
	return spi_flash_info[0].pfWrite(0, dst, length, (unsigned char *)src);
}  int
spi_flw_image(unsigned int chip, unsigned int flash_addr_offset,
	      unsigned char *image_addr,
	      unsigned int image_size)  {
	    KDEBUG
	    ("spi_flw_image: chip=%x; flash_addr_offset=%x; image_addr=%x; image_size=%x\n",
	     chip, flash_addr_offset, (unsigned int)image_addr, image_size);
	return spi_flash_info[chip].pfWrite(chip, flash_addr_offset,
					     image_size, image_addr);
}

int
spi_flw_image_mio_8198(unsigned int cnt, unsigned int flash_addr_offset,
		       unsigned char *image_addr,
		       unsigned int image_size)  {
	    KDEBUG
	    ("spi_flw_image_mio_8198: cnt=%x; flash_addr_offset=%x; image_addr=%x; image_size=%x\n",
