...
void prepare_txpkt(int etherport, Int16 type, Int8 * destaddr, Int8 * data,
		    Int16 len);
...
extern int flashread(unsigned long dst, unsigned int src,
...
static struct irqaction irq_eth15 =
    { eth_interrupt, 0, 15, "eth0", NULL, NULL };

//--------------------------------------------------------------------------------------------
    
void
eth_interrupt(int irq, void *dev_id, struct pt_regs *regs) 
 { 
...
void
getmacandip(unsigned char *mac, unsigned char *ip) 
 { 
unsigned char tmpbuf[6];
...
if (flashread((unsigned long)tmpbuf, CURRENT_SETTING_OFFSET, 6) ==
		 0) {
...
flashread((unsigned long)buf, CURRENT_SETTING_OFFSET + 6,
			     len);
...
/*Just a start address, and the data length*/ 
    void
prepare_txpkt(int etherport, Int16 type, Int8 * destaddr, Int8 * data,
	      Int16 len) 
 { 
