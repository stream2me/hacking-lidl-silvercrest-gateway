...
void
irq_dispatch(int irq_nr, struct pt_regs *regs)
{
	int i, irq = 0;
...
int
setup_IRQ(int irq, struct irqaction *new)
{
	int shared = 0;
...
int
request_IRQ(unsigned long irq, struct irqaction *action, void *dev_id)
{
...
asmlinkage void
do_IRQ(int irqnr, struct pt_regs *regs)
{
...
asmlinkage void
do_reserved(struct pt_regs *regs)
{
