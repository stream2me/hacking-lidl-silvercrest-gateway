...
#define isa_memcpy_fromio(a, b, c)                                             \
	memcpy_fromio((a), isa_slot_offset + (b), (c))
...
#define __IN1(t, s)                                                            \
	static __inline__ t __in##s(unsigned int port)                         \
	{                                                                      \
		t _v;
...
#define __IN(t, m, s)                                                          \
__IN1(t,s) __IN2(m) : "=r" (_v) : "i" (0), "r" (mips_io_port_base+port));      \
	return _v;                                                             \
	}                                                                      \
__IN1(t,s##c) __IN2(m) : "=r" (_v) : "ir" (port), "r" (mips_io_port_base));    \
	return _v;                                                             \
	}                                                                      \
__IN1(t,s##_p) __IN2(m) : "=r" (_v) : "i" (0), "r" (mips_io_port_base+port));  \
	SLOW_DOWN_IO;                                                          \
	return _v;                                                             \
	}                                                                      \
...
#define __INS1(s)                                                              \
	static inline void __ins##s(unsigned int port, void *addr,             \
				    unsigned long count)                       \
	{
...
#define __OUTS1(s)                                                             \
	static inline void __outs##s(unsigned int port, const void *addr,      \
				     unsigned long count)                      \
	{
...
#define wpreg_bwrite(val, addr)                                                \
	do {                                                                   \
		(*(volatile unsigned char *)((addr) + WP_PORT_BASE)) =         \
		    (unsigned char)(val);                                      \
	} while (0)

#define wpreg_hwrite(val, addr)                                                \
	do {                                                                   \
		(*(volatile unsigned short *)((addr) + WP_PORT_BASE)) =        \
		    (unsigned short)(val);                                     \
	} while (0)

#define wpreg_wwrite(val, addr)                                                \
	do {                                                                   \
		(*(volatile unsigned long *)((addr) + WP_PORT_BASE)) =         \
		    (unsigned long)(val);                                      \
	} while (0)
...
static inline unsigned char in_8(const unsigned char *addr)
{
	return *KSEG1ADDR(addr);
}

static inline void out_8(unsigned char *addr, unsigned int val)
{
	*KSEG1ADDR(addr) = (unsigned char)val;
}

static inline unsigned short in_16(const unsigned short *addr)
{
	return (le16_to_cpu(*KSEG1ADDR(addr)));
}

static inline void out_16(unsigned short *addr, unsigned int val)
{
	*KSEG1ADDR(addr) = cpu_to_le16((unsigned short)val);
}

static inline unsigned int in_32(const unsigned int *addr)
{
	return (le32_to_cpu(*KSEG1ADDR(addr)));
}

static inline void out_32(unsigned int *addr, unsigned int val)
{
...
#define inb(port)                                                              \
	((__builtin_constant_p((port)) && (port) < 32768) ? __inbc(port)       \
							  : __inb(port))
...
#define inb_p(port)                                                            \
	((__builtin_constant_p((port)) && (port) < 32768) ? __inbc_p(port)     \
							  : __inb_p(port))
...
#define inw(port)                                                              \
	((__builtin_constant_p((port)) && (port) < 32768) ? __inwc(port)       \
							  : __inw(port))
...
#define inw_p(port)                                                            \
	((__builtin_constant_p((port)) && (port) < 32768) ? __inwc_p(port)     \
							  : __inw_p(port))
...
#define inl(port)                                                              \
	((__builtin_constant_p((port)) && (port) < 32768) ? __inlc(port)       \
							  : __inl(port))
...
#define inl_p(port)                                                            \
	((__builtin_constant_p((port)) && (port) < 32768) ? __inlc_p(port)     \
							  : __inl_p(port))
