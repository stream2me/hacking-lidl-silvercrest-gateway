...
#define write_32bit_cp3_general_register(register, value)                      \
	__asm__ __volatile__("mtc3\t%0," STR(register) "\n\t"                  \
						       "nop"                   \
			     :                                                 \
			     : "r"(value));
...
#define write_32bit_cp3_control_register(register, value)                      \
	__asm__ __volatile__("ctc3\t%0," STR(register) "\n\t"                  \
						       "nop"                   \
			     :                                                 \
			     : "r"(value));
...
#endif
/* On Lexra, CP0 $20 is CCTL (Cache Control), not XCONTEXT as in std MIPS */
#define CP0_CCTL $20
...
#define __read_32bit_c0_register(source, sel)                                  \
	({                                                                     \
		int __res;                                                     \
		if (sel == 0)                                                  \
			__asm__ __volatile__("mfc0\t%0, " STR(source) "\n\t"   \
					     : "=r"(__res));                   \
		else                                                           \
...
/*
 * Macros to access the Lexra coprocessor 0 register include:
 * ESTATUS, ECAUSE, INTVEC, CVSTAG, BPCTL, WMPCTL, WMPSTATUS, WMPVADDR
 */

#define LX0_ESTATUS $0
#define LX0_ECAUSE $1
#define LX0_INTVEC $2
#define LX0_CVSTAG $3
#define LX0_BPCTL $4
...
#ifndef __read_32bit_lxc0_register
#define __read_32bit_lxc0_register(source, sel)                                \
	({                                                                     \
		int __res;                                                     \
		if (sel == 0)                                                  \
			__asm__ __volatile__("mflxc0\t%0, " STR(source) "\n\t" \
					     : "=r"(__res));                   \
		else                                                           \
			__asm__ __volatile__(                                  \
			    ".set\tmips32\n\t"                                 \
...
#define __BUILD_SET_CP3_CON(name, register)                                    \
	static inline unsigned int set_cp3_con_##name(unsigned int set)        \
	{                                                                      \
		unsigned int res;                                              \
                                                                               \
		...
		return res;                                                    \
                                                                               ...
                                                                               \
	static inline unsigned int clear_cp3_con_##name(unsigned int clear)    \
	{                                                                      \
		unsigned int res;                                              \
                                                                               \
		res = read_32bit_cp3_control_register(register);               \
		...
		return res;                                                    \
                                                                               ...
                                                                               \
	static inline unsigned int change_cp3_con_##name(unsigned int change,  \
							 unsigned int new)     \
	{                                                                      \
		unsigned int res;                                              \
                                                                               \
