/*
 * RTL8196E BSP Chip Definitions
 * Copyright (c) 2006 Realtek Semiconductor Corporation
 * Authors: Jwsyu (jwsyu@realtek.com), Tony Wu (tonywu@realtek.com.tw)
 * Adapted for Linux 5.10 & RTL8196E: Jacques Nilo (2025)
 *
 * Chip-level header file for RTL8196E.
 *
...
/* #include <linux/autoconf.h> */  /* Removed in kernel 5.4 - auto-included by kbuild */

/*
 * Register access macros
 *
 * Security warning (2025-11-21): These macros use volatile pointers WITHOUT
 * proper memory barriers, which can cause:
 * - Instruction reordering by compiler/CPU
 * - Cache coherency issues on multi-core systems
 * - Race conditions in interrupt mask registers
 *
 * For shared state (interrupt masks, atomic counters), use:
 * - Explicit memory barriers (mb/rmb/wmb)
 * - Spinlocks for read-modify-write operations
 * - Proper writel/readl accessors (see rtl_nic.c)
 *
 * This is the SAME issue fixed in drivers/clocksource/timer-rtl819x.c where
 * __raw_writel/__raw_readl were replaced with writel/readl for proper barriers.
 */
